...
let year = 2012;
let yearc = (year % 4 == 0 && year % 100 != 0) || year % 400 == 0;
let yearD = yearc ? "윤년" : "평년";
if (yearc) {
  console.log(`${year}년도는 ${yearD}입니다.`);
}
...
//return하는 문장만 실행문에 있으면 중괄호{}와 함께 return은 반드시 생략해야 한다. (중괄호 생략하려면)
let call2 = (a) => a;
...
2) 클로저는 outer(외부) 함수보다 중첩 함수가 더 오래 유지되는 경우 중첩 함수는 
    이미 생명주기가 종료한 외부함수의 변수를 참조할 수 있다. 이러한 중첩함수가 클로저(closure)이다.
...
//(1) 기본 파라미터(default parameter) : E56-2015
...
//(2) 나머지 파라미터(Rest parameter) : E56-2015
// - 파라미터 갯수를 가변으로 사용할 수 있도록 제공해주는 연산자이다.
// - spread operator(점 3개(...))를 맨 마지막 피라미터로 사용해주면 된다.
