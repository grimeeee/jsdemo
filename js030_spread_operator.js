...
// Spread Operatpoor = 스프레드 연산자 = 전개연산자 = 펼침연산자
//형태 (...변수)
...
//1. 배열 복사(Array Copy)
let arr1 = [1, 2, 3];
let arr2 = arr1; //얇은 복사(shallow copy)__주소 전달
console.log(`arr1: ${arr1}`);
console.log(`arr2: ${arr2}`);

arr2.push(4); //주소가 복사가 되었기 때문에 둘 다 값이 바뀜 ( 영향 O )
console.log(`arr1: ${arr1}`); //arr1: 1,2,3,4
console.log(`arr2: ${arr2}`); //arr2: 1,2,3,4

let arr3 = [1, 2, 3];
let arr4 = [...arr3]; //깊은 복사 ( 영향 X )
...
function display(...params) {
  return params.reduce((sum, a) => {
    return sum + a; //중괄호 생략하면 return도 같이생략 ( {return sum )
  });
}
...
console.log(Math.max(...arr13)); //50 (주소 복사)
